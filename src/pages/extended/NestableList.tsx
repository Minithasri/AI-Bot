...
const ListMovable = ({ item }: { item: number }) => {
  return (
    <div className="rounded cursor-grab px-6 py-3 border border-gray-200 dark:border-gray-600">
      Item {item}
    </div>
  );
};
...
const ListMovable2 = ({ item, classname }: { item: number; classname: string }) => {
  return (
    <div
      className={`${
        item === 2 ? `${classname} bg-danger text-white font-bold` : ""
      } rounded cursor-grab px-6 py-3 border border-gray-200 dark:border-gray-600`}
    >
      Item {item}
    </div>
  );
