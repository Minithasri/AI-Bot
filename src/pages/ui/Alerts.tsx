              ...
              <div
                ...
                className={`${
                  defaultAlertVariants[idx].isBG
                    ? `border-0 text-white bg-${defaultAlertVariants[idx].variant}`
                    : `bg-${color}/10` + ` text-${color}`
                } ${
                  color === "light" ? "!text-slate-900" : ""
                }  border border-${color}/20 text-sm rounded - md py-3 px-5`}
                role="alert"
              >
                <span className="font-bold">{color.charAt(0).toUpperCase() + color.slice(1)}</span>{" "}
        ...
        <div className="pt-5">
          <div className="space-y-4">
            {(colors || []).map((color, idx) => (
              <div
                key={idx}
                id={`dismiss-example-${idx}`}
                className={`${
                  dismissibleAlertVariants[idx].isBG
                    ? `border-0 text-white bg-${dismissibleAlertVariants[idx].variant}`
                    : `bg-${color}/10` + ` text-${color}`
                } border   border-${color}/20 rounded-md py-3 px-5 flex justify-between items-center`}
              >
                <p>
                ...
                <button
                  className="text-xl/[0]"
                  type="button"
                  onClick={() => handleDismiss(`dismiss-example-${idx}`)}
                >
        ...
        <div className="pt-5">
          <div className="space-y-4">
            {(colors || []).map((color, idx) => (
              <div
                key={idx}
                className={`text-${color} border border-${color}/20 text-sm rounded-md py-3 px-5`}
                role="alert"
              >
                This is a{" "}
                <span className="font-bold">{color.charAt(0).toUpperCase() + color.slice(1)}</span>{" "}
        ...
        <div className="pt-5">
          <div className="space-y-4">
            {(colors || []).map((color, idx) => (
              <div
                key={idx}
                className={`bg-${color}/10 text-${color} border border-${color}/20 text-sm rounded-md py-3 px-5`}
                role="alert"
              >
        ...
        <div className="pt-5">
          <div className="space-y-4">
            {(iconAlertVariants || []).map((item, idx) => (
              <div
                key={idx}
                className={`bg-${item.variant}/10 text-${item.variant} border border-${item.variant}/20 text-sm rounded-md py-3 px-5`}
              >
