                ...
                <Tab.List as="nav" className="flex space-x-3 border-b">
                  {(tabContents || []).map((tab) => (
                    <Tab
                      key={tab.title}
                      className={({ selected }) =>
                        `py-4 px-1 inline-flex items-center gap-2 border-b-2 border-transparent -mb-px transition-all text-sm whitespace-nowrap text-gray-500 hover:text-primary ${
                          selected
                            ? "border-b-primary font-semibold border-primary text-primary hover:text-primary"
          ...
          <div className="p-6">
            ...
            <div className="pt-5">
              <Tab.Group>
                <Tab.List
                  as="nav"
                  className="relative z-0 flex border rounded-xl overflow-hidden dark:border-gray-600"
                >
                  {(tabContents || []).map((tab, idx) => (
                    <Tab
                      key={idx}
                      type="button"
                      className={({ selected }) =>
                        ` relative min-w-0 flex-1 bg-white first:border-l-0 border-l border-b-2 py-4 px-4 text-gray-500 text-sm font-medium text-center overflow-hidden hover:bg-gray-50 focus:z-10 dark:bg-gray-800 dark:border-l-gray-700 dark:border-b-gray-700 dark:hover:bg-gray-700 dark:hover:text-gray-400 ${
                          selected
          ...
          <div className="p-6">
            ...
            <div className="pt-5">
              <Tab.Group>
                <Tab.List
                  as="nav"
                  className="flex space-x-2 border-b border-gray-200 dark:border-gray-600"
                >
                  {(tabContents || []).map((tab, idx) => (
                    <Tab
                      key={idx}
                      className={({ selected }) =>
                        `dark:bg-gray-800 dark:border-b-gray-800 -mb-px py-3 px-4 inline-flex items-center gap-2 text-sm font-medium text-center border text-gray-500 rounded-t-lg  ${
                          selected
                            ? "text-primary bg-white  border-b-transparent dark:bg-gray-800 dark:border-b-gray-800 dark:text-white"
                            : "text-gray hover:text-gray-700 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400"
                        }`
                      }
                    >
                      {tab.title}
                ...
                <Tab.List as="nav" className="flex space-x-2">
                  {(tabContents || []).map((tab, idx) => (
                    <Tab
                      ...
                      type="button"
                      className={({ selected }) =>
                        ` py-3 px-4 inline-flex items-center gap-2 text-sm font-medium text-center  rounded-lg  dark:hover:text-gray-400 ${
                          selected ? "bg-primary text-white" : "text-gray-500 hover:text-primary"
                        }`
                      }
                    >
                      {tab.title}
                ...
                <Tab.List as="nav" className="flex space-x-2">
                  <Tab
                    ...
                    className={({ selected }) =>
                      ...
                      } flex-auto py-3 px-4 inline-flex justify-center items-center gap-2 text-center text-sm font-medium  rounded-lg  dark:hover:text-gray-400`
                    }
                  >
                    Tab 1
                  </Tab>
                    ...
                    <Tab.List as="nav" className="flex md:flex-col gap-2 space-y-2">
                      {(tabContents || []).map((tab, idx) => (
                        <Tab
                          key={idx}
                          className={({ selected }) =>
                            `btn ${selected ? "bg-primary text-white" : "bg-transparent"}`
                          }
                        >
                          {tab.title}
                        </Tab>
