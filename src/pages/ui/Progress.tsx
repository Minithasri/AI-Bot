      ...
      <div className="p-6">
        ...
        <div className="flex flex-col gap-3">
          {(heightProgress || []).map((item, idx) => (
            <div
              key={idx}
              className={`flex w-full h-${item.height} bg-gray-200 rounded-full overflow-hidden dark:bg-gray-700`}
            >
              <div
                className={`flex flex-col justify-center overflow-hidden ${item.variant}`}
                role="progressbar"
                style={{ width: `${item.valueNow}%` }}
    ...
    <div className="card">
      <div className="p-6">
        ...
        <div className="w-full h-4 bg-black/10 rounded-full">
          <div
            className="bg-danger h-4 rounded-s-full w-7/12 animate-strip"
            style={{
              backgroundImage:
          ...
          {(colors || []).map((color, idx) => {
            return (
              <div key={idx} className="w-full h-4 bg-black/10 rounded-full">
                <div
                  className={`${color} h-4 rounded-s-full animate-strip"`}
                  style={{
                    backgroundImage:
                      "linear-gradient(45deg,hsla(0,0%,100%,.15) 25%,transparent 0,transparent 50%,hsla(0,0%,100%,.15) 0,hsla(0,0%,100%,.15) 75%,transparent 0,transparent)",
                    backgroundSize: "1rem 1rem",
